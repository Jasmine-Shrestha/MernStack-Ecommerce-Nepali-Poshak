  ...
  for(const items in cartItems){
    for(const item in cartItems[items]){
      if (cartItems[items][item] > 0) {
       // const itemInfo = structuredClone(products.find(products.find(product => product._id === items)))
...
case 'stripe':
 ...
 // { Authorization: `Bearer ${token}` , Origin: window.location.origin}
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
     {/** ----------left side------ */}
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <input required onChange={onChangeHandler} name='email' value={formData.email} className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type="email" placeholder='Email name' />
      ...
      {/** -----------RIGHT SIDE-------- */}
      <div className='mt-8'>
      <div className='mt-8 min-w-80 flex flex-col gap-12'>
        <CartTotal/>
...
<div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
  ...
  <img className='h-8 mx-8' src={assets.stripe} alt="" />
...
<p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
  ...
  <img className='h-8 mx-8' src={assets.razorpay} alt="" />
...
{/** button tag when clicked navigated to order page*/}
